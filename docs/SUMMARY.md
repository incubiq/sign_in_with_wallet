# Table of contents

* [Sign-in with Wallet (SIWW)](README.md)

## Specifications

* [Abstract](specifications/creating-your-first-project.md)
* [Motivation](specifications/creating-your-first-task.md)
* [Open source libraries](specifications/advanced-permissions.md)
* [Security considerations](specifications/security.md)

## Use Cases

* [NodeJS - Backend implementation](use-cases/nodejs-backend-implementation/README.md)
  * [1. Setup passport-wallet](use-cases/nodejs-backend-implementation/todo.md)
  * [2. Connect to SIWW](use-cases/nodejs-backend-implementation/2.-connect-to-siww.md)
  * [3. Manage your authentication token](use-cases/nodejs-backend-implementation/3.-manage-your-authentication-token.md)
  * [4. Add authentication routes](use-cases/nodejs-backend-implementation/4.-add-authentication-routes.md)
  * [5. Code the views](use-cases/nodejs-backend-implementation/5.-code-the-views.md)
  * [6. Full sample app](use-cases/nodejs-backend-implementation/5.-full-sample-app.md)
* [ReactJS - connect to backend](use-cases/reactjs-connect-to-backend/README.md)
  * [1. Add a login route](use-cases/reactjs-connect-to-backend/1.-add-a-login-route.md)
  * [2. Redirect the route to your backend](use-cases/reactjs-connect-to-backend/2.-redirect-the-route-to-your-backend.md)
* [JS - Native calls to SIWW libs](use-cases/js-client-implementation/README.md)
  * [1. Initialize SIWW client lib](use-cases/js-client-implementation/initialize-siwc-client-lib.md)
  * [2. Connect to a Wallet](use-cases/js-client-implementation/connect-with-wallet.md)
  * [3. Create a message](use-cases/js-client-implementation/signing-messages.md)
  * [4. Send a message](use-cases/js-client-implementation/signing-messages-1.md)
  * [5. React JS App](use-cases/js-client-implementation/5.-react-js-app.md)
* [Authenticating as an end-user](use-cases/authenticating-as-an-end-user/README.md)
  * [1. Have a wallet available](use-cases/authenticating-as-an-end-user/1.-have-a-wallet-available.md)
  * [2. Connecting and signing](use-cases/authenticating-as-an-end-user/2.-connecting-and-signing.md)
